...
/**
 * A 나라가 B 나라를 침공하였습니다. B 나라의 대부분의 전략 자원은 아이기스 군사 기지에 집중되어 있기 때문에 A 나라는 B
 * 나라의 아이기스 군사 기지에 융단폭격을 가했습니다.
 *
 ...
 *
 * B 나라는 특정 x 좌표에서 y 축에 수평이 되도록 미사일을 발사하며, 발사된 미사일은 해당 x 좌표에 걸쳐있는 모든 폭격 미사일을
 * 관통하여 한 번에 요격할 수 있습니다.
 *
 * 단, 개구간 (s, e)로 표현되는 폭격 미사일은 s와 e에서 발사하는 요격 미사일로는 요격할 수 없습니다.
 *
 * 요격 미사일은 실수인 x 좌표에서도 발사할 수 있습니다.
 *
 * 각 폭격 미사일의 x 좌표 범위 목록 targets이 매개변수로 주어질 때, 모든 폭격 미사일을 요격하기 위해 필요한 요격 미사일 수의
 * 최솟값을 return 하도록 solution 함수를 완성해 주세요.
 *
 * 제한사항:
 * - 1 <= targets.length <= 500_000
...
public class 요격시스템 {
    ...
    public static int solution(int[][] targets) {
        ...
        while (!minHeap.isEmpty()) {
            count++;
            int[] currentTarget = minHeap.poll();
            while (!minHeap.isEmpty() && minHeap.peek()[0] < currentTarget[1]) {
                minHeap.poll();
            }
        }
